...
function StatBox({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: "0.85rem", fontWeight: 800, fontFamily: "'JetBrains Mono', monospace", color, marginTop: 1 }}>{value}</div>
    </div>
  );
}
  ...
  return (
    <div style={{ background: "var(--bg-primary)", borderRadius: "var(--radius-sm)", padding: "6px 8px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
        ...
        <span style={{ fontSize: "0.6rem", fontFamily: "'JetBrains Mono', monospace", color }}>{pct.toFixed(1)}%</span>
      </div>
