    ...
    for (const action of actions) {
      try {
        ...
        const allowedTypes = [
          ...
          "DIPLOMACY", "MILITARY", "RENAME", "FORUM_POST", "PLANT_CROP", "DOMESTICATE_ANIMAL", "PROPOSE_TRADE", "ACCEPT_TRADE",
        ];
  ...
  app.get("/api/v2/system-prompt", async (request: AuthenticatedRequest, reply) => {
    return reply.send({
      ...
      available_actions: [
        ...
        "SET_POLICY", "DIPLOMACY", "MILITARY", "RENAME", "FORUM_POST", "PLANT_CROP", "DOMESTICATE_ANIMAL", "PROPOSE_TRADE", "ACCEPT_TRADE",
      ],
    });
  });
