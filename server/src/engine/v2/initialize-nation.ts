...
export async function initializeNation(nationId: number): Promise<{ humans: number; territory_km2: number }> {
  const nation = await getOne<{ founding_lat: number; founding_lng: number }>(
    "SELECT founding_lat, founding_lng FROM nations WHERE id = $1",
    [nationId]
  );
  ...
  if (!lat || !lng) {
    const cell = await getOne<{ seed_lat: number; seed_lng: number }>(
      `SELECT seed_lat, seed_lng FROM mesh_cells
       WHERE owner_id IS NULL AND cell_type = 'land'
    ...
    lng = cell.seed_lng;
    await query(
      "UPDATE nations SET founding_lat = $1, founding_lng = $2 WHERE id = $3",
      [lat, lng, nationId]
    );
  }
